...
Use a `max` aggregation to find the largest amount of money successfully received by Jill (`user_id` of 4). Return her `user_id` and that amount.
...
A transaction can only have a `sender_id` or a `recipient_id`, but not both. The presence of one or the other indicates whether money is going into or out of the user's account.

- The `sender_id` will be present for any transactions where the user in question (`user_id`) is receiving money (from the sender) so the `recipient_id` will be `NULL`.
- The `recipient_id` will be present for any transactions where the user in question (`user_id`) is sending money (to the recipient) so the sender_id will be `NULL.`
